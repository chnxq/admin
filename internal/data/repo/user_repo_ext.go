...
func (r *userRepo) userRepoCustomList(ctx context.Context, builder *ent.UserQuery, req *paginationv1.PagingRequest) (*paginationv1.PagingRequest, error) {
	return r.applyUserRelationFilters(ctx, builder, req)
}

func parseUserRelationFilterID(field string, condition *paginationv1.FilterCondition) (uint32, bool, error) {
	if condition == nil || condition.GetField() != field || condition.GetOp() != paginationv1.Operator_EQ {
		return 0, false, nil
	}
	...
	if err != nil {
		return 0, false, fmt.Errorf("invalid %s filter value: %w", field, err)
	}
	return uint32(parsed), true, nil
}

func uniqueUint32Values(items []uint32) []uint32 {
	if len(items) <= 1 {
		return items
	}
...
func intersectUint32Values(left []uint32, right []uint32) []uint32 {
	if len(left) == 0 || len(right) == 0 {
		return nil
	}
	index := make(map[uint32]struct{}, len(left))
	...
	result := make([]uint32, 0, len(right))
	...
	return uniqueUint32Values(result)
}

func (r *userRepo) listUserIDsByOrgUnitID(ctx context.Context, orgUnitID uint32) ([]uint32, error) {
	rows, err := r.entClient.Client().UserOrgUnit.Query().
		Where(
	...
	return uniqueUint32Values(ids), nil
}

func (r *userRepo) listUserIDsByPositionID(ctx context.Context, positionID uint32) ([]uint32, error) {
	rows, err := r.entClient.Client().UserPosition.Query().
		Where(
...
func (r *userRepo) listUserIDsByRoleID(ctx context.Context, roleID uint32) ([]uint32, error) {
	rows, err := r.entClient.Client().UserRole.Query().
		Where(
...
func (r *userRepo) applyUserRelationFilters(ctx context.Context, builder *ent.UserQuery, req *paginationv1.PagingRequest) (*paginationv1.PagingRequest, error) {
	if req == nil || req.GetFilterExpr() == nil || len(req.GetFilterExpr().GetConditions()) == 0 {
		return req, nil
	}

	remaining := make([]*paginationv1.FilterCondition, 0, len(req.GetFilterExpr().GetConditions()))
	var relationUserIDs []uint32
	...
	for _, condition := range req.GetFilterExpr().GetConditions() {
		...
		if orgUnitID, matched, err := parseUserRelationFilterID("org_unit_id", condition); err != nil {
			return nil, err
		} else if matched {
			userIDs, err := r.listUserIDsByOrgUnitID(ctx, orgUnitID)
			if err != nil {
				return nil, err
			}
			relationUserIDs = userIDs
			if len(relationUserIDs) == 0 {
				builder.Where(user.IDEQ(0))
			}
			continue
		}

		if positionID, matched, err := parseUserRelationFilterID("position_id", condition); err != nil {
			return nil, err
		} else if matched {
			userIDs, err := r.listUserIDsByPositionID(ctx, positionID)
			if err != nil {
				return nil, err
			}
			if relationUserIDs == nil {
				relationUserIDs = userIDs
			} else {
				relationUserIDs = intersectUint32Values(relationUserIDs, userIDs)
			}
			if len(relationUserIDs) == 0 {
				builder.Where(user.IDEQ(0))
			}
		...
		if roleID, matched, err := parseUserRelationFilterID("role_id", condition); err != nil {
			return nil, err
		} else if matched {
			userIDs, err := r.listUserIDsByRoleID(ctx, roleID)
			if err != nil {
				return nil, err
			}
			if relationUserIDs == nil {
	...
	if relationUserIDs != nil {
		if len(relationUserIDs) == 0 {
			...
			builder.Where(user.IDIn(relationUserIDs...))
		}
	}

	cloned := protoClonePagingRequest(req)
	if cloned == nil {
		return req, nil
	}
	filterExpr := cloned.GetFilterExpr()
	if filterExpr == nil {
		return cloned, nil
	}
	filterExpr.Conditions = remaining
	cloned.FilteringType = &paginationv1.PagingRequest_FilterExpr{FilterExpr: filterExpr}
	return cloned, nil
}

func protoClonePagingRequest(req *paginationv1.PagingRequest) *paginationv1.PagingRequest {
	if req == nil {
		return nil
	}
	cloned := *req
